...
// Only accept POST requests (we are changing data)
...
try {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        // No row matched — either the id does not exist or it was already completed
        http_response_code(404);
        echo json_encode([
            'success' => false,
            'message' => 'Habit not found or already completed.'
        ]);
    }
} catch (PDOException $e) {
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Failed to update habit: ' . $e->getMessage()]);
}
